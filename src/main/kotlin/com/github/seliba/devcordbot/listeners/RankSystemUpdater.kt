    ...
    fun onMessageSent(event: DevCordGuildMessageReceivedEvent) {
        ...
        transaction {
            ...
            Users.update(
                ...
                (it as UpdateBuilder<Users>)[Users.lastUpgrade] = Instant.now()
                val xpToLevelup = XPUtil.getXpToLevelup(user.level)
                if (user.experience >= xpToLevelup) {
                    it[Users.experience] = user.experience + 5 - xpToLevelup
                    it[Users.level] = user.level + 1
                    newLevel++
                } else {
                    it[Users.experience] = user.experience + 5
                }
            }
        }
